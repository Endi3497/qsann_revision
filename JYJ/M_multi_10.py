...
def load_mnist_local(raw_dir):
    ...
    train_images = read_images(os.path.join(raw_dir, "train-images-idx3-ubyte.gz"))
    train_labels = read_labels(os.path.join(raw_dir, "train-labels-idx1-ubyte.gz"))
    test_images = read_images(os.path.join(raw_dir, "t10k-images-idx3-ubyte.gz"))
    test_labels = read_labels(os.path.join(raw_dir, "t10k-labels-idx1-ubyte.gz"))
    return (train_images, train_labels), (test_images, test_labels)

# MNIST 데이터 불러오기
MNIST_RAW_DIR = "/home/junyeollee/QSANN/codes/QSANN_revision/data/MNIST/raw"
mnist_files = [
...
if all(os.path.exists(os.path.join(MNIST_RAW_DIR, f)) for f in mnist_files):
    (X_train_full, y_train_full), (X_test_full, y_test_full) = load_mnist_local(MNIST_RAW_DIR)
else:
...
y_train = to_categorical_np(y_train, num_classes=10)
y_test = to_categorical_np(y_test, num_classes=10)
...
def count_qkv_params(text_classifier):
    qkv_total = 0
    for qsal in text_classifier.Qnn.qsal_lst:
        qkv_total += qsal.init_params_Q.numel()
        qkv_total += qsal.init_params_K.numel()
        qkv_total += qsal.init_params_V.numel()
    return qkv_total

qkv_params = count_qkv_params(model)
linear_params = sum(p.numel() for p in model.final_layer.parameters())
print(f"QKV circuit params: {qkv_params}")
print(f"Final linear params: {linear_params}")
...
def multi_accuracy(preds, y):
    ...
    y_tensor = y_tensor.to(pred_index.device).long()

    correct_pred = (pred_index == y_tensor).float()
    return correct_pred.mean()

LOG_DIR = "/home/junyeollee/QSANN/codes/QSANN_revision/JYJ/logs"
MODEL_DIR = "/home/junyeollee/QSANN/codes/QSANN_revision/JYJ/models"
os.makedirs(LOG_DIR, exist_ok=True)
os.makedirs(MODEL_DIR, exist_ok=True)

model_save_path = os.path.join(MODEL_DIR, "qsann_classifier_multi_10.pth")
results_path = os.path.join(LOG_DIR, "result_10.csv")
...
if labels_np.ndim > 1:
    ...
    labels_bin = labels_np
else:
    labels_idx = labels_np.astype(int)
    labels_bin = label_binarize(labels_idx, classes=np.arange(preds_np.shape[1]))
...
auroc = roc_auc_score(labels_idx, preds_np, multi_class="ovr")

# Calculate Precision, Recall, and F1 Score (macro average for multi-class)
precision, recall, f1, _ = precision_recall_fscore_support(labels_idx, predicted_classes, average='macro')

# Calculate AUPRC (Area Under the Precision-Recall Curve) for each class
auprc = average_precision_score(labels_bin, preds_np, average="macro")
...
metrics = {
    "acc": float(acc.item() if torch.is_tensor(acc) else acc),
    "ROC": float(auroc),
