...
# Dataset settings
DATASET_CHOICE=${DATASET_CHOICE:-"pcam"}          # mnist|fmnist|cifar10|medmnist|pcam
DATASET_LABELS=${DATASET_LABELS:-"0 1"}            # space-separated labels (required for mnist/fmnist)
SAMPLES_PER_LABEL=${SAMPLES_PER_LABEL:-600}        # per-label cap; unset/empty to keep all
...
MEASUREMENT=${MEASUREMENT:-"correlations"}         # statevector|correlations|xyz|z_zz_ring
...
NUM_CLASSES=${NUM_CLASSES:-"2"}                     # set to override inferred class count (e.g., 5)
...
EARLY_STOP=${EARLY_STOP:-"--early-stop"}                       # set to "--early-stop" to enable
EARLY_STOP_PATIENCE=${EARLY_STOP_PATIENCE:-10}
EARLY_STOP_MIN_DELTA=${EARLY_STOP_MIN_DELTA:-0.0}
NO_POS_WEIGHT=${NO_POS_WEIGHT:-""}                 # set to "ㅊ--no-pos-weight" to disable pos_weight
...
SAVE_STATEVECTOR_EPOCH=${SAVE_STATEVECTOR_EPOCH:-1}
...

{
  ...
  $NO_BALANCE_SAMPLER \
  $SAVE_STATEVECTOR \
  --save-statevector-epoch "$SAVE_STATEVECTOR_EPOCH" \
  --seed "$SEED" \
  --log-dir "$LOG_DIR" \
  --model-dir "$MODEL_DIR" \
  --run-name "$RUN_NAME"
